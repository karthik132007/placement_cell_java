...
public class InterviewDB {
    ...
    public static boolean updateInterviewStatus(int interviewId, String status, String notes) {
        try (Connection c = connecton.getConnection()) {
            // First get interview details for placement creation if status is "Accepted"
            PreparedStatement ps1 = c.prepareStatement(
                    "SELECT a.s_id, a.driveId, s.name, s.email, co.name AS cname, d.lpa " +
                    "FROM interviews i " +
                    "JOIN applications a ON i.application_id = a.A_id " +
                    "JOIN student s ON a.s_id = s.rollnum " +
                    "JOIN drive d ON a.driveId = d.D_id " +
            ...
            if (rs.next()) {
                ...
                if (rsCompany.next()) {
                    companyId = rsCompany.getInt("companyId");
                }
            }
            ...
            if (success && studentRoll != null && "Accepted".equals(status)) {
                ...
                ps3.setDouble(4, salary * 100000); // Convert LPA to actual salary
                ps3.executeUpdate();
